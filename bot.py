...
async def send_dev_log(guild_id, error_message, severity="ERROR"):
    ...
    if dev_chan_id:
        ...
        if channel:
            ...
            # FIXED: Using triple quotes to prevent SyntaxErrors from line breaks
...
def home():
    ...
    return f"V3.0 Final Patched Online | msgs: {bot_stats['messages_processed']} | Uptime: {uptime}h"
...
class GroqBot(discord.Client):
    ...
    async def setup_hook(self):
        ...
        print(f"Logged in as {self.user} | V3.0 Final Patched")

    @tasks.loop(minutes=15)
    async def status_task(self):
        statuses = [
            discord.Activity(type=discord.ActivityType.watching, name="the digital horizon"),
            discord.Game(name="with SQLite Threads"),
            discord.Activity(type=discord.ActivityType.listening, name="API heartbeats"),
            discord.Game(name="with LLaMA 3.3"),
            discord.Activity(type=discord.ActivityType.watching, name="autonomous research")
        ]
...
async def info(interaction: discord.Interaction):
    ...
    embed.set_footer(text="⚙️ Built by yathin | Patched Build")
    ...
    if bot.user.mentioned_in(message) or is_active or isinstance(message.channel, discord.DMChannel):
        ...
        if toggles.get("auto_research", True) and await needs_research(user_text):
            async with message.channel.typing():
                await message.reply("⏳ *Autonomous protocol active. Pulling live data...*")
                def quick_scrape():
        ...
        sys_prompt = {"role": "system", "content": f"{sys_content}{JAILBREAK_PROMPT} [Time: {ist_time.strftime('%I:%M %p')}]"}
