...
class SlashCommands(commands.Cog):
    ...
    @app_commands.command(name="personality", description="Set bot persona")
    async def set_personality(self, interaction: discord.Interaction, bio: str):
        await interaction.response.defer()
        update_config(interaction.guild_id, personality=bio if bio.lower() != "default" else "")
        await interaction.followup.send(f"🎭 Persona Updated.")
